

export  const names = {
    'B zone': ['name1', 'name2', 'name3', 'Muhsin', 'Rashid', 'Hamid Muhammed', 'Dildhar', 'Junaid tsy', 'Zainul Abid', 'Razi', 'Safwan kdy', 'Hashir', 'Sayyid Hashbal', 'Sahal Mueen',
                'Fawas Aralam', 'Noufal Vkd', 'Safwan Tvm', 'Muhammed Iyad', 'Swalih', 'Hisham Banglore', 'Naeem', 'Davood', 'Muhammed Bh', 'Shamveel',
                'Hisham Mlp', 'Sufair', 'Ramees', 'Sayyd Bishrul hafi', 'Mishab', 'Musthafa P', 'Habeeb Paravoor', 'Jaseem', 'Ameen tvm', 'Bishr','Midlaj thathur',
                'Safwan Malayamma', 'Jaseel', 'Sharbas', 'Mishab pynr'],


    'C zone': ['name1', 'name2', 'name3','Muhammad Hameed', 'Sahil', 'Hijaz', 'Rashid Malayamma', 'Shibili', 'Salman Manalipuzha', 'Swabah', 'Habeeb Wayanad', 'Sinan Ap',
                'Juhan', 'Abdul Hafeez', 'Shaheem', 'Salmanul Faris', 'Favas Mp', 'Muhthasim Billah' ,'Ajeer ','Ziyad', 'Nihal Kp', 'Zuhair', 'Thasneem',
                'Musthafa V', 'Shuhaib Ktdy', 'Farhan', 'Sahad', 'Shuhaib prpngdi', 'Riyas', 'Hafeez P', 'Muhammed Ali', 'Ameen Elad', 'Aslam Kdduvally','Sinan TVM',
                'Thajudeen', 'Mubashir', 'Aslam Poonur', 'Nihal Kayankulam'],


    'Y zone': ['name1', 'name2', 'name3','Hubaib', 'Muhammad Aliyar', 'Shafar', 'Shahabaz', 'Mansoor', 'Sabith', 'Bilal', 'Mushfiq', 'Faris', 'Ashkar', 'Fahad', 'Junaid kpm', 'Shamnad', 
               'Shuaib Ayyoob','Mujthaba', 'Shahid', 'Salman thirurangadi', 'Noufal Abdussalam', 'Fazlu Rahman','Labeeb','Shaheer', 'Abdul Raheem', 'Adil Ashraf', 'Abdul Qader','Umar Ali','Unais Nizar' ,'Yoonus','Ismail',
                'Shurahbeel','Mubarak', 'Swalahudheen','Mueenudheen'],
};